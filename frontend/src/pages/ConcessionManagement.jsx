...
const ConcessionManagement = () => {
    ...
    // Auto-switch to first permitted tab once user is loaded
    useEffect(() => {
        if (!user) return;
        const isSA = user.role === 'superadmin' || user.role === 'admin';
        const perms = user.permissions || [];
        const tabOrder = [
            { tab: 'request',   key: 'concession_raise_request' },
            { tab: 'approvals', key: 'concession_approvals' },
            { tab: 'approvers', key: 'concession_approvers' },
            { tab: 'reports',   key: 'concession_reports' },
        ];
        const currentAllowed = isSA || perms.includes(tabOrder.find(t => t.tab === activeTab)?.key);
        if (!currentAllowed) {
            const first = tabOrder.find(t => isSA || perms.includes(t.key));
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <header className="bg-white border-b px-6 py-3 flex justify-between items-center shrink-0">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl shadow-inner border border-gray-200">
                        {(isSuperAdmin || (user?.permissions || []).includes('concession_raise_request')) && (
                            <button
                        ...
                        {(isSuperAdmin || (user?.permissions || []).includes('concession_reports')) && (
                            <button
                            ...
                            <div className="p-4 border-b bg-gray-50 space-y-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        />
                                        {isSearching && (
                                            <div className="absolute right-3 inset-y-0 flex items-center gap-0.5">
