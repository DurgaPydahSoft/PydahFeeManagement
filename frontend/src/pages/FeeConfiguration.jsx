...
const FeeConfiguration = () => {
    ...
    const [structForm, setStructForm] = useState({
        ...
        academicYear: '2024-2025', studentYear: '', amount: ''
    });
    ...
    const [appContext, setAppContext] = useState({
        college: '', course: '', branch: '', studentYear: '', feeHeadId: '', academicYear: '2024-2025'
    });
    ...
    const handleEditHead = (h) => {
        setHeadForm({ name: h.name, description: h.description });
        setEditHeadId(h._id);
    };
    ...
    const activeStructSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                ...
                const formattedBulk = {};
                Object.keys(bulkAmounts).forEach(key => {
                    if (bulkAmounts[key] !== '' && bulkAmounts[key] !== undefined && bulkAmounts[key] !== null) {
                        formattedBulk[key] = Number(bulkAmounts[key]);
                    }
                });

                const hasBulk = Object.keys(formattedBulk).length > 0;
                const hasSingle = structForm.amount !== '' && structForm.amount !== undefined;

                if (!hasBulk && !hasSingle) {
                    alert("Please enter amount for at least one year.");
                    return;
                }

                const payload = { ...structForm, yearAmounts: formattedBulk };
                await axios.post(`${import.meta.env.VITE_API_URL}/api/fee-structures`, payload);
                setMessage('Fee Definitions created successfully!');
            }
            fetchStructures();
    ...
    // Edit entire row (Propagate context + years to form)
    const handleEditRow = (row) => {
        setStructForm({
            ...
            studentYear: '', // Reset single year focus
            amount: ''
        });

        // Populate bulkAmounts
        const newBulk = {};
        if (row.years) {
            Object.keys(row.years).forEach(y => {
                newBulk[y] = row.years[y].amount;
            });
        }
        setBulkAmounts(newBulk);
        setEditingId(null); // Ensure we are in "Create/Bulk" mode to update all
        window.scrollTo(0, 0);
    };
    ...
    const fetchStudentsForApplicability = async () => {
        ...
        try {
            ...
            // We search the local 'structures' state or fetch from API. 
            // Local state is faster.
            const template = structures.find(s =>
    ...
    structures.filter(s => {
        ...
        if (!grouped[key]) grouped[key] = { ...s, feeHeadName: s.feeHead?.name, feeHeadId: s.feeHead?._id, years: {}, allIds: [] };
        grouped[key].years[s.studentYear] = { id: s._id, amount: s.amount };
                ...
                {activeTab === 'heads' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="md:col-span-2 bg-white p-5 rounded-lg shadow-sm">
                            ...
                            <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gray-50"><tr><th className="p-2">Name</th><th className="p-2">Desc</th><th className="p-2 text-right">Action</th></tr></thead>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-100 h-fit">
                            ...
                            <form onSubmit={activeStructSubmit} className="space-y-4 text-sm">
                                {/* Context Selection reused from original code... simplified here for readability */}
                                <select className="w-full border p-2 rounded" value={structForm.college} onChange={e => { setStructForm({ ...structForm, college: e.target.value }); setBulkAmounts({}) }} required><option value="">Select College</option>{colleges.map(c => <option key={c}>{c}</option>)}</select>
                                <select className="w-full border p-2 rounded" value={structForm.course} onChange={e => { setStructForm({ ...structForm, course: e.target.value }); setBulkAmounts({}) }} required disabled={!structForm.college}><option value="">Select Course</option>{(structForm.college ? Object.keys(metadata[structForm.college] || {}) : []).map(c => <option key={c}>{c}</option>)}</select>
                                ...
                                {editingId ? (
                                    ...
                                    <div className="bg-blue-50 p-3 rounded grid grid-cols-2 gap-2">
                                        <p className="col-span-2 text-xs font-bold text-gray-600">Amounts by Year</p>
                                        {((structForm.college && structForm.course) ? Array.from({ length: metadata[structForm.college][structForm.course].total_years || 4 }, (_, i) => i + 1) : []).map(y => (
                                            <input key={y} className="border p-1 rounded text-sm" placeholder={`Year ${y}`} value={bulkAmounts[y] || ''} onChange={e => setBulkAmounts({ ...bulkAmounts, [y]: e.target.value })} />
                                        ))}
                                    </div>
                                )}
                                    ...
                                    {groupedArray.map((row, i) => (
                                        <tr key={i} className="hover:bg-gray-50 group/row">
                                            ...
                                            <td className="p-3 text-xs text-gray-500">
                                                <div className="font-bold">{row.course}</div>
                                                <div>{row.branch}</div>
                                                <div className="text-[10px] uppercase">{row.college}</div>
                                            </td>
                                            {[1, 2, 3, 4].map(y => (
                                                <td key={y} className="p-2 text-center text-gray-700">
                                                    {row.years[y] ? `₹${row.years[y].amount.toLocaleString()}` : '-'}
                                                </td>
                                            ))}
